...
// ── Concurrency ────────────────────────────────────────────────────────────────
// Both scanning phases now run several pages in parallel within one shared
// browser (cheap — they're tabs, not separate browser processes) instead of
// doing everything one page at a time. This is the main speed lever: the old
// fully-serial detail loop was the dominant cost on large jobs (thousands of
// listings at several seconds each, one at a time). Tunable via Railway env
// vars if you have more/less RAM available than the defaults assume:
//   ZONE_CONCURRENCY   — parallel tabs while scanning search zones (default 4)
//   DETAIL_CONCURRENCY — parallel tabs while fetching phone/website/email (default 6)
// Lower these if you see "Chromium launch failed" / OOM errors on your plan;
// raise them if you have RAM to spare and want it even faster.
const ZONE_CONCURRENCY = Math.max(1, parseInt(process.env.ZONE_CONCURRENCY || "4", 10));
const DETAIL_CONCURRENCY = Math.max(1, parseInt(process.env.DETAIL_CONCURRENCY || "6", 10));

// How many zones/listings to process per "generation" before recycling the
// browser. Recycling still happens periodically for the same memory-leak
// reasons as before — it just now bounds a batch of concurrent work instead
// of a single page.
const ZONES_PER_GENERATION = ZONE_CONCURRENCY * 8;
const DETAILS_PER_GENERATION = DETAIL_CONCURRENCY * 15;
...
async function runScrapeJob(job) {
  ...
  try {
    ...
    jlog(job, `🚀 Launching browser (${ZONE_CONCURRENCY}x parallel zones)…`);
    ...
    // Remaining zones run ZONE_CONCURRENCY-at-a-time in tabs sharing one
    // browser, processed in generations so the browser can still be recycled
    // periodically for memory (see notes above the concurrency constants).
    let zi = 1;
    while (zi < gridPoints.length && !job.cancelled && seen.size < maxResults) {
      ...
      await runPool(genPoints, ZONE_CONCURRENCY, async (pt, localIdx) => {
    ...
    if (fetchDetails && !job.cancelled) {
      jlog(job, `📞 Fetching phone, website & email for ${allListings.length} places (${DETAIL_CONCURRENCY}x parallel)…`);
      ...
      while (di < allListings.length && !job.cancelled) {
        ...
        await runPool(genItems, DETAIL_CONCURRENCY, async (listing) => {
