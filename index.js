...
// How often to kill and relaunch Chromium during a long-running job.
// A single browser session held open for hundreds of zones / thousands of
// detail-page visits leaks memory over hours until new launches start timing
// out (this is what was causing "Chromium launch failed: Timeout 60000ms
// exceeded" after the server had been up for a while). Recycling the process
// periodically keeps memory bounded instead of letting it climb for the
// entire job.
const RECYCLE_EVERY_ZONES = 20;
const RECYCLE_EVERY_DETAILS = 40;
...
async function runScrapeJob(job) {
  ...
  try {
    ...
    for (let i = 0; i < gridPoints.length; i++) {
      ...
      // Recycle the browser periodically — a single Chromium process held
      // open across hundreds of zones slowly leaks memory until new launches
      // start timing out. Restarting it every N zones keeps memory bounded.
      if (i > 0 && i % RECYCLE_EVERY_ZONES === 0) {
        jlog(job, `   🔄 Recycling browser (${i}/${gridPoints.length} zones done)…`);
    ...
    if (fetchDetails && !job.cancelled) {
      ...
      for (let i = 0; i < allListings.length; i++) {
        ...
        if (i > 0 && i % RECYCLE_EVERY_DETAILS === 0) {
          jlog(job, `   🔄 Recycling browser (${i}/${allListings.length} details done)…`);
          await safeCloseBrowser(browser);
          ({ browser: b, context } = await launchBrowser());
          browser = b;
          page = await context.newPage();
        ...
        try {
          const d = await withTimeout(getPlacePhone(page, listing.mapsUrl), 120000, "getPlacePhone");
          phone = d.phone || "";
          website = d.website || "";
        } catch (e) {
          jlog(job, `   ⏱️ Skipped phone/website for "${listing.name}" (${e.message})`);
          // The page may still be wedged mid-navigation — swap in a fresh one
          // (best-effort close, never block on it) so the hang can't carry
          // over and stall the next listing too.
          try {
        ...
        if (website) {
          try { email = await withTimeout(getEmailFromWebsite(context, website), 40000, "getEmailFromWebsite"); } catch (_) {}
        }
        if (phone) withPhone++;
